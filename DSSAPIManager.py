...
SETTINGS_PATH = r"C:\distr\DSSAPIManager\\"
DELIMETER_LEVEL = '=' * 80
DELIMETER_ULEVEL = '-' * 80
...
def main():
    ...
    mode, debug = select_mode() # Make choice
    # mode, debug = '1', 'Y' # Auto mode: DSSUser+Debug on
            ...
            if inp_choice in ['1', '2', '3', '4', '5', 'q', 'Q']:
                if inp_choice == '1':
                    print("\n", DELIMETER_LEVEL, "\n", "Attempt to sign a test document with the following parameters: ", 
                        "\n", DELIMETER_ULEVEL, sep='')
                    print("Selected user certificate: ", 'Certificate ID: ', SessionDSS.certs[0]['ID'],
                        '; Distinguished Name: ', SessionDSS.certs[0]['DName'], "\n",
                        DELIMETER_ULEVEL, sep=''
                        )
                    ...
                    print("The document was successfully signed!", "\n", DELIMETER_ULEVEL, sep='')
                    print("Verify Sign...")
                ...
                if inp_choice == '2':
                    ...
                    for i in range(len(SessionDSS.certs)):
                        print(DELIMETER_ULEVEL, "\n", "Certificate № ", str(i), '. ', 'Certificate ID: ', 
                            SessionDSS.certs[i]['ID'], ', Distinguished Name: ',
                            SessionDSS.certs[i]['DName'], "\n", DELIMETER_ULEVEL, sep=''
                            )
                    
                    while True:
                        print("\n")
                        cert_choice = input("Select certificate (0-" + str((len(SessionDSS.certs) - 1)) 
                        + "): ")
                        if cert_choice in [str(i) for i in range(len(SessionDSS.certs))]:
                            print("\n", "Selected certificate №", cert_choice, " (",
                                'Certificate ID: ', str(SessionDSS.certs[int(cert_choice)]['ID']),
                                ', Distinguished Name: ', SessionDSS.certs[int(cert_choice)]['DName'], ")",
                                "\n", DELIMETER_LEVEL, "\n", sep='' 
                                )
                            break
                        print("Incorrect value. Please enter value 0 to " + str((len(SessionDSS.certs)) - 1))
                    ...
                    while True:
                        ...
                        doc_type_choice = input("\nSelect DocTypes (0-" + str((len(SessionDSS.doc_types) - 1)) + "): ") 
                        if doc_type_choice in [str(i) for i in range(len(SessionDSS.doc_types))]:
                            print("\n", "Selected DocType: ", doc_type_choice, " (",
                                SessionDSS.doc_types[int(doc_type_choice)], ")", "\n", DELIMETER_LEVEL, sep='' 
                                )
                            break
                        print("Incorrect value. Please enter value 0 to " + str((len(SessionDSS.doc_types)) - 1))

                    while True:
                        print(DELIMETER_LEVEL, "\n", "\nSelect CADESType: ")
                        for i in range(len(SessionDSS.cades_types)):
                            print(str(i) + '. ' + SessionDSS.cades_types[i])
                        cades_types_choice = input("\nSelect (0-" + str((len(SessionDSS.cades_types) - 1)) + "): ") 
                        if cades_types_choice in [str(i) for i in range(len(SessionDSS.cades_types))]:
                            print("\n", "Selected CADESType: ", cades_types_choice,
                                " (", SessionDSS.cades_types[int(cades_types_choice)], ")", "\n", 
                                DELIMETER_LEVEL, sep='' 
                                )
                            break
                        print("Incorrect value. Please enter value 0 to ", (len(SessionDSS.cades_types)) - 1, sep='') 
                    
                    print(DELIMETER_LEVEL)
                    SessionDSS._pincode = get_pin()
                    print(DELIMETER_LEVEL)
                    
                    SessionDSS.sign_doc(cert_num=int(cert_choice), doc_type=int(doc_type_choice), cades_type=int(cades_types_choice))
                    ...
                    while chk_path:
                        ...
                        if os.path.exists(spath):
                            ...
                            while True:
                                for i in range(len(SessionDSS.doc_types)):
                                    print(str(i) + '. ' + SessionDSS.doc_types[i])
                                doc_type_choice = input("\nSelect Signature Type (0-" + str((len(SessionDSS.doc_types) - 1)) + "): ") 
                                if doc_type_choice in [str(i) for i in range(len(SessionDSS.doc_types))]:
                                    print("\n", "Selected Signature Type: ", doc_type_choice, " (",
                                        SessionDSS.doc_types[int(doc_type_choice)], ")", "\n", DELIMETER_LEVEL, sep='' 
                                        )
                                    break
                                print("Incorrect value. Please enter value 0 to " + str((len(SessionDSS.doc_types)) - 1))
...
'''                     )
                        ...
                        if inp_choice in ['1', '2', '3', '4', '5', '6', 'q', 'Q']:  
                            ...
                            if inp_choice == '2':
                                print("\n", DELIMETER_LEVEL, sep='')
                                print_attribute(attr_name="ADFS Token value: ", attr_val=SessionDSS._ADFS_token)
                                print(DELIMETER_LEVEL)

                            if inp_choice == '3':
                                print("\n", DELIMETER_LEVEL, sep='')
                                print_attribute(attr_name="DSS Token value: ", attr_val=SessionDSS._DSS_token)
                                print(DELIMETER_LEVEL)

                            if inp_choice == '4':
                                print("\n", DELIMETER_LEVEL, sep='')
                                print_attribute(attr_name="Path to save the last signed document: ", attr_val=SessionDSS.user_sfile_path)
                                print_attribute(attr_name="Last Signed Document(Base64): ", attr_val=SessionDSS.last_signed_doc)
                                print(DELIMETER_LEVEL)

                            if inp_choice == '5':
                                print("\n", DELIMETER_LEVEL, sep='')
                                print_attribute(attr_name="Document Path: ", attr_val=SessionDSS.user_file_path)
                                print_attribute(attr_name="Document(Base64) before signing: ", attr_val=SessionDSS.user_file_base64)
                                print(DELIMETER_LEVEL)
    ...
    if mode == '2':
        ...
        try:
            ...
            print("Authorization Error:", str(err))
        print('Enter username to manage (local user login DSS). Example: HQ00-SC-ECM-TEST')
...
class SessionCls:
    def __init__(self, config_dict, debug):
        ...
        self.get_certs_url = config_dict['APIEndpoints']['GetCerts']
        ...
        self._adfs_client_id = config_dict['Credentials']['ADFS']['client_id']
        self._adfs_client_secret = config_dict['Credentials']['ADFS']['client_secret']
        self._DSS_client_id = config_dict['Credentials']['DSS']['client_id']
        self._DSS_client_secret = config_dict['Credentials']['DSS']['client_secret']
        self._adfs_scope = config_dict['Credentials']['ADFS']['scope']
        ...
        self._pincode = config_dict['SignConfig']['PinCode']
    ...
    def get_certs(self):
        self.certs = dss_get_req(get_url=self.get_certs_url, 
    ...
    def sign_doc(self, cert_num, doc_type, cades_type):
        ...
        self.last_signed_doc, status = sign_doc_req(
            sign_url=self.sign_url, DSS_token=self._DSS_token, TSP_address=self.TSP_address, 
            cert_id=self.certs[cert_num]['ID'], doc_type=self.doc_types[doc_type], cades_type=self.cades_types[cades_type], 
            doc=enc_doc, pincode=self._pincode
            )
        if status == 200:
            if self.user_file_path == "":
                pass
                ...
                try:
                    base64_decode_and_save(base64signdoc=self.last_signed_doc, path_to_save=self.user_file_path + '.sig')
                    self.user_sfile_path = self.user_file_path + '.sig'
                except ValueError:
                    print(f"Save signed document error: {ValueError}")
                    
                print("The document was successfully signed and saved in: ", self.user_file_path + '.sig')
            
            if self.debug:
                print(DELIMETER_ULEVEL, "\n", "Signed Document (Base64 encoded): ", 
                    "\n", self.last_signed_doc[:30], "......", self.last_signed_doc[-30:], "\n", 
                    DELIMETER_ULEVEL, sep=''
                    )
        else:
            print("Document signing failed! Response received:\n", "Status code: ", status, "\nMessage:\n", self.last_signed_doc, sep='')
    ...
    def verify_sign(self, sdoc_path, signature_type):
        ...
        verify_res, status = verify_sign_req(verify_sign_url=self.verify_sign_url, DSS_token=self._DSS_token, 
                                    doc=enc_doc, sign_type=self.doc_types[signature_type])
    ...
    def get_DSS_code_mTLS(self):
        ...
        r = requests.get(self.getDSScode_url, params=args, 
    ...
    def get_DSS_dm_token(self, user):
        ...
        encoded_sub_token = base64.b64encode(json.dumps(sub_token_head, indent=4).encode('utf-8')).decode('utf-8') + "." + base64_url_encode(json.dumps(sub_token_payload, indent=4).encode('utf-8')).decode('utf-8') + "."
    ...
    def new_cert_issue(self):
        ...
        while not chk:
            ...
            for i in range (len(self.policy['CAPolicy'][0]['NamePolicy'])):
                if self.policy['CAPolicy'][0]['NamePolicy'][i]['OID'] == "1.2.643.100.3":
                    print("Attention! The SNILS format must contain 11 digits in a row!. Example: 17816373499")
                inp = input("Введите " + self.policy['CAPolicy'][0]['NamePolicy'][i]['Name'] + 
                " (OID: " + self.policy['CAPolicy'][0]['NamePolicy'][i]['OID'] + "): "
                )
                cert_args[self.policy['CAPolicy'][0]['NamePolicy'][i]['OID']] = [inp, self.policy['CAPolicy'][0]['NamePolicy'][i]['Name']]
        ...
        resp, status = new_cert_req(url=self.requests_uri, dss_token=self._DSS_dm_token, 
                                        ca_id=ca_id, templ=cert_templ, attr=cert_args_res,
                                        pin=self._pincode, debug=self.debug
                                        )
        if status == 200:
            if resp['Status'] == 'ACCEPTED':
                print(DELIMETER_LEVEL, "\n", "Certificate issued successfully!\n Information about the issued certificate: \n", sep='')
                print("CertificateID: ", resp['CertificateID'], "\n", 
                    "DistName: ", resp['DistName'], "\n", 
                    "Certificate_in_Base64: ", resp['Base64Request'], "\n", DELIMETER_LEVEL, sep=''
                    )
            else:
                print(DELIMETER_LEVEL, "\n", "Certificate issue status:", resp['Status'], sep='')
                print("Message:\n", resp, "\n", DELIMETER_LEVEL, sep='')
        else:
...
def adfs_auth(auth_url, token_url, client_id, client_secret, redirect_uri, 
            debug, scope=['openid', 'allatclaims']):
    ...
    adfs_session = OAuth2Session(client_id,scope=scope,redirect_uri=redirect_uri, state=state)
    ...
    authorization_url = authorization_url + '&' + 'response_mode=' + response_mode + '&' + 'nonce=' + nonce # FIX ADFS requirements
...
def dss_auth(dss_token_url, client_id, grant_type, mode, client_secret='', token='', subject_token='', code='', redirect_uri='', debug=False):
    headers = {
    ...
    if mode == "3":
        data = data3
        auth = base64.b64encode((client_id + ":" + client_secret).encode('utf-8')).decode('utf-8')
...
def dss_get_req(get_url, dss_token):
    headers = {
...
def sign_doc_req(sign_url, DSS_token, TSP_address, cert_id, doc_type, cades_type, doc, pincode):
